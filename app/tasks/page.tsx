...
import { CheckCircle2, Circle, Smartphone, Clock, DollarSign } from "lucide-react"
import { cn } from "@/lib/utils"

export default function TasksPage() {
  const { tasks, completeTask, submitScreenTime, hasCompletedSetup } = useGame()
  ...
  const handleCompleteTask = (taskId: string) => {
    ...
    if (task) {
      setFeedback(`Great job! You earned $${task.reward.toFixed(2)}! 🎉`)
          ...
          <Card className="shadow-lg">
            ...
            <CardContent className="p-4 space-y-3">
              {choresTasks.map((task) => (
                <TaskItem key={task.id} task={task} onComplete={() => handleCompleteTask(task.id)} />
              ))}
            </CardContent>
          </Card>
          ...
          <Card className="shadow-lg">
            ...
            <CardContent className="p-4 space-y-4">
              <p className="text-sm text-muted-foreground">
                Enter your daily screen time to earn bonus money! Less screen time = more rewards!
              </p>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    step="0.5"
                  />
                  <Button onClick={handleScreenTimeSubmit} className="bg-purple-600 hover:bg-purple-700">
                    Submit
                  </Button>
                </div>
              </div>
        ...
        <Card className="mt-6 bg-gradient-to-r from-green-50 to-emerald-50 border-green-200">
          <CardContent className="p-6">
            ...
            <ul className="space-y-2 text-green-700">
              ...
              <li>📱 Reduce screen time to earn bigger bonuses</li>
              <li>🔄 Tasks reset daily - check back tomorrow!</li>
...
function TaskItem({
  ...
  task: { id: string; title: string; description: string; reward: number; completed: boolean }
  onComplete: () => void
}) {
  return (
    <div
    ...
    >
      <button
        ...
        className={cn(
          ...
          task.completed ? "text-green-500" : "text-muted-foreground hover:text-primary",
        )}
      >
